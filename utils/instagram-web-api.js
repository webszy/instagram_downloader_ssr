...
class Instagram {
    ...
    async _getQueryHash(username = '123') {
        let jsFile, result
        await this.request('/' + username).then(html => {
                let str = html.split('<link rel="preload"')
                let str2 = str[1].split('href=')
                let idx = str2[1].indexOf('as="script"')
                jsFile = str2[1].substring(0, idx).replace('"', '').replace('"', '')
            })
            // console.log(jsFile)
        await this.request(jsFile).then(js => {
                let qhList = js.match(/(queryId:\s?)(\S*),/g)
                let qhList2 = qhList.map(element => {
